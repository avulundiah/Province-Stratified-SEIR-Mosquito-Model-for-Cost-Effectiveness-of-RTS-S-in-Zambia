function simulate_model()
    ...
    mean_epsilon_A = 0.70; % Vaccine efficacy
    mean_nu_A      = 0.5;  % Vaccination rate
    
    % Uncertainty assumptions (adjust as appropriate):
    % We assume normal distributions around the means, truncated if necessary.
    sd_beta  = 0.02;       % e.g., ~10% relative uncertainty
    sd_sigma = 0.005;      % adjust based on plausible incubation uncertainty
    sd_gamma = 0.005;      % adjust based on plausible infectious-period uncertainty
    sd_epsilon = 0.05;     % vaccine efficacy uncertainty
    sd_nu = 0.1;           % vaccination rate uncertainty
    
    % Empirical data uncertainty: assume ±5% (normal) around each observed value
    obs_sd = 0.05 * obs;   % standard deviation for empirical error bars
    ...
    for k = 1:Nsim
        ...
        % Vaccine scenario
        epsilon_A_samp = normrnd(mean_epsilon_A, sd_epsilon);
        epsilon_A_samp = min(max(epsilon_A_samp, 0), 1);
        nu_A_samp = normrnd(mean_nu_A, sd_nu);
    ...
    IA_median = prctile(IA_runs, 50, 1);
    ...
    [rmseA, r2A] = fit_metrics(obs, IA_median);
    [rmseB, r2B] = fit_metrics(obs, IB_median);
    fprintf('\nWith Vaccine (median):   RMSE = %.2f, R^2 = %.3f\n', rmseA, r2A);
    ...
    %%fill_yA = [IA_upper, fliplr(IA_lower)];
    %%hA = fill(fill_x, fill_yA, colorAlpha([0.1 0.6 1.0], 0.2), 'EdgeColor', 'none');
    % Shaded confidence for no vaccine
    fill_yB = [IB_upper, fliplr(IB_lower)];
    hB = fill(fill_x, fill_yB, colorAlpha([0.9 0.9 1], 0.2), 'EdgeColor', 'none');
    
    % Plot median curves
    %%plot(years, IA_median, '-o', 'Color', [0.1 0.6 1.0], 'LineWidth', 2);
    plot(years, IB_median, ':', 'Color', [1.0 0.1 0.3], 'LineWidth', 3);
    % Plot observed data with error bars (±1.96*sd)
   % err = 1.96 * obs_sd;
    %errorbar(years, obs, err, 'ks', 'LineWidth', 1.5, 'MarkerSize', 6, 'CapSize', 10);
    plot(years, obs, 'bo','MarkerSize', 10, ...
        'MarkerFaceColor', 'b')
    
    xlabel('Year'); ylabel('Infectious cases per 1000');
    title('Model Validation with 95% CI: With vs Without Vaccine');
    box on;
    grid on;
    %legend([hA, hB], {'95% CI (With Vaccine)', '95% CI (No Vaccine)'}, 'Location', 'northeast');
    legend('95% CI ','Model prediction','Emperical data')
    % Add median curves to legend
    %%legend_entries = {'95% CI (With Vaccine)', '95% CI (No Vaccine)', ...
             %%         'Median (With Vaccine)', 'Median (No Vaccine)', 'Observed ±95% CI'};
    % Re-plot median and obs in legend:
    %%plot_handles = [hA, hB, ...
                   %%plot(NaN,NaN,'-o','Color',[0.1 0.6 1.0],'LineWidth',2), ...
                    %%plot(NaN,NaN,'--s','Color',[1.0 0.3 0.3],'LineWidth',2), ...
                    %%errorbar(NaN,NaN,0, 'ks','LineWidth',1.5,'MarkerSize',6,'CapSize',10)];
%%legend(plot_handles, legend_entries, 'Location', 'northeast');
   %%legend('Model with vaccine','Model with no vaccine','Emperical data')
    grid on;
    hold off;
    box on
end
...
function c = colorAlpha(rgb, alpha)
    ...
    c = rgb;
    % We set FaceAlpha when calling fill; here for clarity we return rgb and set alpha in fill:
    % In this code, we pass alpha directly in fill: colorAlpha is placeholder to package rgb.
    % Alternatively, could modify to return an RGBA if supported. 
    % Here: fill(..., 'FaceColor', rgb, 'FaceAlpha', alpha)
    % But since we passed alpha to fill directly, this utility is minimal.
end
